...
#     }
#
#     # Coreference and discourse processing variables
...
#                         scorefs_map[scoref] = dcoref
#                         rev_scorefs_map[dcoref] = scoref
#                     else:
...
#                             exist_antecedent = any(scorefs_map[scoref] in doc_sent for doc_sent in all_document_sents[:di])
#                             if exist_antecedent:
...
#                                 })
#                     if len(smention) == 1 and smention[0] in ["he", "she", "him", "her", "his", "they", "them", "their", "it", "this", "that", "those", "these"]:
#                         errors["IncomCorefEval"]["count"] += 1
#                         errors["IncomCorefEval"]["details"].append({
#                             "summary_sentence": senti,
#                             "mention": smention,
...
#                     smention.append(sword)
#                 if dword != sword:
#                     while dword != sword:
#                         if "[" in dword:
#                             dcoref = dword
#                         elif dcoref is not None and "span>" in dword:
#                             if dcoref in rev_scorefs_map:
#                                 scorefs[rev_scorefs_map[dcoref]].append(dmention)
#                             else:
#                                 if dcoref not in dscorefs:
#                                     dscorefs[dcoref] = []
#                                 dscorefs[dcoref].append(dmention)
#                                 if len(dscorefs[dcoref]) == 1 and di != 0:
#                                     exist_antecedent = any(dcoref in doc_sent for doc_sent in all_document_sents[:di])
#                                     if exist_antecedent:
#                                         errors["IncomCorefEval"]["count"] += 1
#                                         errors["IncomCorefEval"]["details"].append({
#                                             "summary_sentence": senti,
...
#                     dmention.append(dword)
#                 dptr += 1
#                 sptr += 1
#
#         # Check discourse errors
#         if locations[si][1] == 0:
...
#                     })
#             else:
#                 for key in ["also", "however", "but", "clearly", "meanwhile", "not only", "not just", "on another", "then", "moreover"]:
